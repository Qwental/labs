...
int main()
{
    ...
    ERRORS_EXIT_CODES error = calculate_coefs_polinom_with_shift(2.0, &result, degree_n_of_polindrom, 1.0, -2.0, 1.0);
    if (error == E_SUCCESS)
    {
        printf("Результат: \n");
        int i;
    ...
    error = calculate_coefs_polinom_with_shift(3.0, &result, degree_n_of_polindrom, -2.0, 1.0, -3.0, 0.0, 1.0);
    if (error == E_SUCCESS)
    {
        printf("Результат: \n");
        int i;
